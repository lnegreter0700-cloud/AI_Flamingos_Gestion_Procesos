...
| Exportar corte y listado de conciliación diaria | Omisión del arqueo diario por sobrecarga operativa acumulando saldos sin corte formal | Pérdida de deslinde de responsabilidad entre custodios de turno e imposibilidad de rastreo (Crítico) | Revisión y cotejo manual del reporte de corte previo por el Coordinador de Tesorería mediante lista de verificación (checklist) y firma en bitácora física de entrega-recepción al inicio del turno | Detectivo / Manual |
...
2. **Clasificación de Controles:** Los controles se distribuyen entre modalidades preventivas automáticas y detectivas manuales, asegurando la trazabilidad tanto en el software `App Web Flamingos` como en los protocolos de supervisión física y bitácoras de entrega-recepción.
